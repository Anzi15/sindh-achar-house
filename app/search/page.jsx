...
function filterProductsByQuery(products, query, matchAll = false) {
...
export async function generateMetadata({ searchParams }) {
  ...
  return {
    ...
    description: `Looking for the best match to ${query}? here are the results you need!`,
  };
}
...
export default async function ProductsPage({ searchParams }) {
  ...
  if (query) {
    products = filterProductsByQuery(products, query, false);
  }
      ...
      {products.length > 0 ? (
        <>
          <div className="px-6 flex w-full justify-between items-center">
            <h1 className="md:text-[3rem] text-3xl uppercase text-left md:py-18 py-8">
              {query || "Products"}
            </h1>
        ...
        <>
          <div className="min-h-[70vh]">
            <Image
              src="https://i.ibb.co/KWjrFgZ/d6dr79w7vh-O38a-INu-H.png"
