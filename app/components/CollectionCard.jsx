...
const CollectionCard = ({image, collectionName, collectionSlug}) => {
  return (
    <Link href={`/collection/${collectionSlug}`} className="md:w-1/5 w-[45%] md:m-0 my-4 flex flex-col group relative group aspect-square">
    <Image
      src={image}
    ...
    <h3 className="text-brandRed font-semibold  group-hover:hidden text-2xl uppercase font-sans text-center">
      {collectionName}
    </h3>
  </Link>
  )
